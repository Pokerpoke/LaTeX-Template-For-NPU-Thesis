...
ENDIF()
	...
	\section{USRP}
		...
		\subsection{UHD}
			\par 安装完成GNU Radio以后可以会自动安装UHD相关软件，USRP B200 mini需要UHD3.9.0及以上版本，使用版本过低会导致USRP无法识别。UHD提供了一些简单信号发生函数，能够用来实现一些简单的测试。
			\par 首次使用需要执行
			\begin{lstlisting}[ language = sh ]
...
uhd_siggen_gui -f 900000000
	...
	\section{git}
		...
		\subsection{常用命令}
			...
			\par 合并指定分支至当前分支。
			\par 一个好的习惯是在添加新功能或者修复某一个bug时创建一个相应的分支，在该分支中修改，修改完成并测试通过以后合并至主分支，这样可以在不影响主分支的情况进行bug的修复和新功能的添加。
