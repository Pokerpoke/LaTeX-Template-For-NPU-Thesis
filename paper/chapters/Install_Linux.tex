...
\chapter{ARM平台---树莓派}
	\section{简介}
		...
		\par 树莓派使用SD卡存储资料，且拥有一个以太网接口、两个USB接口、支持HDMI（支持声音输出）和RCA端子输出。操作系统采用开源的Linux系统如Debian、ArchLinux，自带Iceweasel、KOffice等软件，能够满足基本的网络浏览、文字处理以及电脑学习的需要。树莓派基金会提供了基于ARM架构的Debian、Arch Linux和Fedora等的发行版供大众下载，支持Python、C语言和Perl等常见编程语言。表\ref{table:params_of_raspi}为树莓派3B+的参数\ucite{ wiki:树莓派}。
		\begin{table}[!htbp]
			\centering
			\caption{树莓派3B+参数}
