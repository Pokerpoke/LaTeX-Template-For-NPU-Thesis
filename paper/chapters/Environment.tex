...
\chapter{环境配置}
	\section{CMake}
		\subsection{简介}
			\par CMake是个开源的跨平台自动化建构系统，它采用与Unix中Make命令相似的建构过程，不同之处在于CMake采用CMakeLists.txt文件而非Makefile来指定建构过程。CMake先产生如Unix的Makefile或Windows Visual C++的projects/workspaces等标准的建构档，然后再依一般的建构方式编译二进制档。CMake可以编译源代码、制做程序库、产生适配器，还可以用任意的顺序建构可执行文件，既支持在当前目录中建构也支持指定输出目录里建构，因此以很容易从同一个源代码目录树中建构出多个二进档\ucite{ wiki:CMake}。
...
ENDIF()
	...
	\section{Python}
		\subsection{简介}
			\par Python，是一种解释型的、面向对象的计算机程序语言，它能够在仅使用标准库的情况下完成许多操作。它的语法简单，使用缩进来定义语句块，这一点与其它大多数程序设计语言使用大括号不一样。Python经常被当作脚本语言用于处理系统管理任务和网络程序编写，它也非常适合完成各种高级任务。Python支持众多编程范式，如函数式编程、泛型编程、面向对象程序设计、命令式程序设计、面向侧面的程序设计等\ucite{ wiki:Python}。
	...
	\section{USRP}
		\subsection{简介}
			\par 通用软件无线电外设（USRP）是由Ettus Research及其母公司National Instruments设计和销售的一系列软件定义无线电设备。由Matt Ettus领导的团队开发，USRP产品系列旨在成为软件无线电相对便宜的硬件平台，已经被研究实验室，大学和业余爱好者广泛使用。大多数USRP通过USB、网卡、PCIe等高速链路连接到主机，主机软件用于控制USRP硬件和发送/接收数据，一些USRP模型还将主机的一般功能与嵌入式处理器集成，从而允许USRP设备以独立的方式运行。USRP系列设计之初便有很高的可修改性，许多产品是开源硬件，所有USRP型号的电路板原理图均可从网站上免费获得，所有USRP产品均采用开源的UHD驱动程序进行控制。USRP通常与GNU Radio软件套件一起使用，以创建复杂的软件定义无线电系统\ucite{ wiki:USRP}。
			\begin{figure}[htbp]
				\centering
...
uhd_siggen_gui -f 900000000
	...
	\section{GNU Radio}
		\label{sec:gnuradio}
		\subsection{简介}
			\par GNU Radio是免费开源的软件无线电开发工具套件。它提供构建软件无线电所需的众多信号运行和处理的模块，用它可以在低成本外部射频（RF）硬件和通用微处理器上或无硬件的模拟环境中实现软件定义无线电。已经有众多业余爱好者，学术机构和商业机构在GNU Radio平台上研究和构建无线通信系统。
			\par GNU Radio可以进行各类信号处理。可以使用它编写应用程序从数据流中获取数据或将数据传输到数据流中，然后使用硬件将其发射出去。GNU Radio含有滤波器、通道编码、解码器等很多单元（GNU Radio中，这些被称作功能块--blocks)，这些单元也都是无线电系统中的常见部件单元。它还具有连接这些功能模块及管理在这些功能块间数据的传输的策略，能够简单的检测框图输入输出的匹配。GNU Radio的应用主要是用Python来编写的。但是其核心信号处理模块是 C++ 在带浮点运算的微处理器上构建的\ucite{what_is_gnuradio}。
