...
sudo apt install libboost-all-dev
	...
	\section{Doxygen}
		\subsection{简介}
			...
			\par 在编写程序的过程中使用符合Doxygen规范的注释块，如有需要，构建时便可以使用Doxygen生成如图\ref{fig:doxygen_gnuradio}的说明文档以及如图\ref{fig:doxygen_dvbt_map}调用关系图。
			\begin{figure}[htb]
				\centering
...
uhd_siggen_gui -f 900000000
	...
	\section{Docker}
		\subsection{简介}
			...
			\par 本设计由于时间问题并未采用Docker来管理系统镜像版本，在修改系统环境的过程中走了不少弯路，后续开发者可以一开始就在Docker的环境下开发软件，或者在安装Ubuntu时使用LVM，创建快照省去因为系统环境配置出错而频繁的重装系统，同时其他开发者可以直接拉取该镜像以进行二次开发。
		% \subsection{构建容器}
	\section{GNU Radio}
		\label{sec:gnuradio}
		\subsection{简介}
